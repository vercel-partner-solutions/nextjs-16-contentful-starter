...
npm install contentful @contentful/rich-text-react-renderer babel-plugin-react-compiler
...
Create a `.env.local` file in the project root to store your Contentful secrets. Use the values you saved from Step 2 (Setting Up Contentful) - you'll find these in the settings for the API key you created earlier.
...
> **Note:** Replace the placeholder values above with your actual Contentful credentials from Step 2.
...
> **Note:** If you haven't created any articles in Contentful yet (from Step 2), the page will load successfully but appear empty with no article cards. This is expected behavior. Create at least two example articles in your Contentful space to see the full experience.
...
### Verify Cache Components are Working

After updating your configuration and restarting the development server, check your terminal output. You should see:

```
▲ Next.js 16.0.10 (Turbopack, Cache Components)
```

The presence of "Cache Components" in the startup message confirms that caching is enabled and working correctly.
...
> **Note:** By default, Vercel has Deployment Protection for all preview deployments. You will need to add the `x-vercel-protection-bypass` header for this webhook to work with protected preview deployments. You can find more information [here](https://vercel.com/docs/deployment-protection/methods-to-bypass-deployment-protection/protection-bypass-automation#using-protection-bypass-for-automation).
...
3. Ensure you're using [Protection Bypass for Automation](https://vercel.com/docs/deployment-protection/methods-to-bypass-deployment-protection/protection-bypass-automation#using-protection-bypass-for-automation) with Preview Deployments.
